    ...
    def turnon_light(self, light, percentage):
        try:
            ...
            self.payload = json.dumps(
                {"entity_id": f"light.{light}", "brightness_pct": f"{percentage}"}
            )
...
def post_message(
    message=None, channel=config["SLACK_DEFAULT_CHANNEL"], blocks=None, files=None
):
    try:
        message = "MPCNC: " + message
        slackapi.chat_postMessage(
            channel=channel, text=message, blocks=blocks, file=files
        )
    except:
